...
switch($_GET['frame']){
	...
	case "shadow":
		...
		if($x7c->permissions['admin_panic']){
			...
			$db->DoQuery("UPDATE {$prefix}rooms SET shadow='$new_shadow' WHERE name='{$_GET[room]}'");
		}
		break;
			...
			if(!in_array($row[0],$x7c->profile['ignored'])){
				...
				if($row[1] == 1){
					...
					if($x7c->permissions['admin_panic']){
						...
						if(!$x7c->room_data['shadow'] || strtolower($x7s->username) == strtolower($row[0]))
							$toout.="$row[2]<br>";
						else
...
if($x7c->permissions['admin_panic']){
	...
	echo '<div id="clean_chat"><a onClick="javascript: do_delete(\'all\')">[Pulisci chats]</a></div>
                      ...
                      <div id="shadow_room"><a id="shadow_link" onClick="javascript: switch_shadow()">'.$shd_txt.'</a></div>';
}
...
<table cellspacing=3 cellpadding=0 style="width: 100%">
	<tr>
		<td style="width: 100%; text-align: center;"><select class="button"
			...
			?>
		</select> <select class="button" name="objects" style="width: 80px;"
			onChange="javascript: return action_select(this);">
		...
		if($x7c->permissions['write_master']){
			...
			onClick=\"document.chatIn.msgi.value ='@_all_@ '; document.chatIn.msgi.focus();\" />";
		}
			
		?> 
		<input name="speech_btn" type="button" class="button" value="Parlato"
			onClick="javascript: speech();" /> <input name="button_send" type="submit" class="send_button" style="cursor: pointer;background: url(<?PHP echo $print->image_path; ?>send.gif);border: none;height: 20px;width: 55px;text-align: center;font-weight: bold;" onMouseOut="this.style.background='url(<?PHP echo $print->image_path; ?>send.gif)'" onMouseOver="this.style.background='url(<?PHP echo $print->image_path; ?>send_over.gif)'" value="<?PHP echo $txt[181]; ?>">
		</td>
	</tr>
</table>
</div>
