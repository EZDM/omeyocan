							...
							function msgSent(){
								...
								if(!message.match(/^@/) && !message.match(/^\*/)){
									if(trim(message).length < <?PHP echo $x7c->settings['min_post'];?>){
										Alert("Il post � troppo corto - deve essere almeno <?PHP echo $x7c->settings['min_post'];?> caratteri");
										return false;
									}
									if(trim(message).length > <?PHP echo $x7c->settings['max_post'];?>){
										Alert("Il post � troppo lungo - sono consentiti al max <?PHP echo $x7c->settings['max_post'];?> caratteri");
										return false;
									}
								}
